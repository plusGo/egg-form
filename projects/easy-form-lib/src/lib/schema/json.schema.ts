import {EtUISchemaItem} from './ui.schema';

export interface EtSchemaDefinition {
  [key: string]: EtSchema;
}

export interface EtSchemaEnum {
  [key: string]: any;
  ...
  /** 组对应的子类 */
  children?: EtSchemaEnum[];
}

export type EtSchemaType = 'number' | 'integer' | 'string' | 'boolean' | 'object' | 'array';

export type EtSchemaEnumType = EtSchemaEnum | number | string | boolean;
...
export interface EtSchema {
   ...
   * 定义属性
   */
  properties?: { [key: string]: EtSchema };
  //////////// 条件类/////////////
  // 未来可能被移除
  ...
  // $schema?: string;
  /** 内部类型定义体 */
  definitions?: EtSchemaDefinition;
  /** 引用定义体 */
  $ref?: string;
  // $schema?: string;
