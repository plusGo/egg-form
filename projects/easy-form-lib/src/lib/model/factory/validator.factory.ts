...
export class AjvSchemaValidatorFactory extends SchemaValidatorFactory {
  ...
  constructor(@Inject(EtFormConfig) private readonly options: EtFormConfig) {
    super();
    this.ajv = new Ajv({
      ...options.ajv,
  ...
  createValidatorFn(schema: EtSchema, extraOptions: { ingoreKeywords: string[]; debug: boolean }): (value: EtValue) => ErrorData[] {
    const ingoreKeywords: string[] = [...(this.options.ingoreKeywords as string[]), ...((extraOptions.ingoreKeywords as string[]) || [])];

    return (value: EtValue): ErrorData[] => {
      try {
